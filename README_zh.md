...
[CoStrict](https://costrict.ai) 是一款基于终端的 AI 编程助手，让你无需离开命令行即可与大语言模型协作完成编码任务。核心能力包括：

- **多种启动方式** — TUI 交互界面（`cs`）、用于脚本和自动化的非交互模式（`cs run "..."`）以及 Web 界面（`cs web`）
- **强大的内置工具** — 文件读写、Shell 执行、代码搜索、网络请求等
- **插件系统扩展** — 通过自定义工具和 MCP 服务器实现无限扩展
- **灵活的配置体系** — 通过 `costrict.json` 管理模型选择、工具权限、主题和快捷键
- **IDE 集成** — 通过 ACP 协议与 Zed、JetBrains、Neovim 深度集成，同时支持 VS Code 扩展

## 什么是 CoStrict Manager？

CoStrict Manager 是专为 CoStrict 打造的现代化 AI 智能体管理平台，提供移动优先的 Web 界面，让你可以在任何设备（包括手机和平板）上管理、控制和使用 CoStrict AI 智能体。

CoStrict Manager 作为 CoStrict CLI 的 Web UI 层，连接到 CoStrict 服务器（端口 5551），并通过后端 API（端口 5003）提供完整的管理界面。
...
- pnpm >= 10.28.1
- Bun >= 1.0
- 已安装并运行 [CoStrict CLI](https://costrict.ai)
...
- **CoStrict 文档**: [https://docs.costrict.ai](https://docs.costrict.ai)
